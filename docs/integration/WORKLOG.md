...
## 2026-09-25 — Phase 3H native runtime closure rerun

A fresh local runtime was provisioned rather than reusing the incomplete checkout. PostgreSQL 16.15 served the dedicated `plandex_phase3h` database at the redacted shape `postgres://plandex_phase3h:***@127.0.0.1:5432/plandex_phase3h?sslmode=disable`. Native binaries were built at `/opt/integration/bin/plandex-server` and `/opt/integration/bin/plandex`; server state used `/var/lib/plandex-phase3h-server`, CLI state used `/var/lib/plandex-phase3h-cli`, and the verified 3,613,922-byte tokenizer remained at `/opt/integration/tiktoken-cache/fb374d419588a4632f3f557e76b4b70aebbca790`.

The first standalone tokenizer setup attempt exposed one concrete deployment defect: importing the dependency-free tokenizer contract through `open_webui` executed the application package initializer and failed when optional `typer` was absent. The deployment scripts now load that module directly and need no Open WebUI Python dependencies. The exact Plandex server image dependencies (including LiteLLM 1.72.6) were installed in an isolated virtual environment; no root requirements or Node workspace was installed. On a completely cold LiteLLM import the native server's ten-second internal proxy deadline fired just before workers became healthy. Leaving the now-healthy proxy running and restarting the server succeeded; this is recorded as a cold-start timing constraint, not hidden as a successful first attempt.

Native migrations ran successfully from an empty database and `/health` returned HTTP 200. With stdin closed, the canonical readiness gate created one user and one org, wrote native auth state, and validated it. CLI/native-home/auth/accounts modes were respectively 0700/0700/0600/0600. A repeated run remained at one user and one org. A copied home with invalid auth and deliberately broad credential modes recovered non-interactively, returned valid auth, repaired both files to 0600, and did not create another account or org.

In a detached worktree at the previously authorized base `9a67af27865d86427546bdacf852136f7d54a812`, native `plandex new --name agent-phase3h-runtime --context-dir .` completed with stdin closed in 13,697 ms. Stable `current --json` reported plan `f06c9b15-e51e-4a00-9685-36a56536c383`, project `7772ab05-d6b3-4159-b2ab-5ca7154ea47d`, name `agent-phase3h-runtime`, branch `main`. After restarting the native server, migrations were up to date, health and readiness passed, and byte-for-byte identical current identity returned; the native database still contained exactly one plan. No token or credential contents were printed. The authenticated control-plane API/restart path remains the prior Phase 3G acceptance recorded above; this rerun specifically reconfirmed the fresh native database/server/auth/new/current/restart boundary.
